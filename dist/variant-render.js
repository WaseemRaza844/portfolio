(function () {
  ...
  const byIds = (items, ids) =>
    ids
      .map((id) =>
        items.find((item) => item.id === id && item.published !== false),
      )
      .filter(Boolean);
  const tags = (items) =>
    '<div class="tags">' +
  ...
  document
    ...
    ?.setAttribute("content", variant.intro);
  ...
  document.getElementById("variant-professional-title").textContent =
  ...
  document.getElementById("variant-image").alt =
    "Temporary technology workspace visual for " +
    shared.shortName +
    "'s portfolio";
  document.getElementById("variant-designations").innerHTML =
    variant.designations.map((x) => "<span>" + esc(x) + "</span>").join("");

  document.getElementById("variant-proof").innerHTML = variant.proof
    .map(
      (item) =>
        '<div><span class="proof-kicker">' +
        esc(item[0]) +
        "</span><strong>" +
        esc(item[1]) +
        "</strong><small>" +
  ...
  document.getElementById("variant-skills").innerHTML = variant.skills
    .map(
      (item, index) =>
        '<article class="research-card reveal visible"><span class="num">0' +
        (index + 1) +
        "</span><h3>" +
        esc(item[0]) +
        "</h3><p>" +
  ...
  document.getElementById("variant-projects").innerHTML = projects
    .map(
      (item) =>
        '<article class="project-card reveal visible"><div class="detail-card-top"><p class="project-year">' +
        esc(item.date) +
        '</p><span class="project-kind">' +
        esc(item.category) +
        "</span></div><h3>" +
        esc(item.title) +
        "</h3><p>" +
  ...
  const experience = shared.experience.filter((item) =>
    item.profiles.includes(variantId),
  );
  document.getElementById("variant-experience").innerHTML = experience
    .map(
      (item) =>
        '<article class="timeline-item reveal visible"><div class="dot"></div><p class="date">' +
        esc(item.period) +
        "</p><div><h3>" +
        esc(item.role) +
        " <span>· " +
        ...
        "</span></h3><p>" +
        esc(item[variantId] || item.generic) +
        "</p></div></article>",
    )
    .join("");

  const publications = byIds(portfolio.publications, variant.publicationIds);
  document.getElementById("variant-publications").innerHTML = publications
    .map((item) => {
      const content =
        '<span class="year">' +
        esc(item.year) +
        '</span><div><p class="pub-venue">' +
        esc(item.venue) +
        "</p><h3>" +
        esc(item.title) +
        "</h3><p>" +
        ...
        "</span>";
      return item.url
        ? '<a class="pub reveal visible" href="' +
            esc(item.url) +
            '" target="_blank" rel="noopener">' +
            content +
            "</a>"
        : '<article class="pub reveal visible">' + content + "</article>";
    })
    .join("");

  document.getElementById("variant-education").innerHTML = shared.education
    .map(
      (item) =>
        "<article><span>" +
        esc(item.period) +
